...
void CLaserOdometry2DNode::LaserCallBack(const sensor_msgs::msg::LaserScan::SharedPtr new_scan)
{
  if (GT_pose_initialized)
  {
    ...
    else
    {
      ...
      // The laser pose is read from TF exactly once, so it must actually be there.
      // robot_state_publisher needs /joint_states before base_link -> laser exists,
      // which can easily land after the first scan; initializing anyway would latch a
      // zero (invalid) quaternion as the laser pose and silently corrupt all odometry.
      if (!setLaserPoseFromTf())
      {
        RCLCPP_WARN_THROTTLE(get_logger(), *this->get_clock(), 2000,
                             "Laser pose not on TF yet - deferring RF2O initialization.");
        return;
      }
...
{  
  ...
  {
    RCLCPP_WARN_THROTTLE(get_logger(), *this->get_clock(), 2000, "%s", ex.what());
    // Bail out rather than converting a default-constructed (zero) quaternion below.
    return false;
  }
...
void CLaserOdometry2DNode::process()
{
  ...
  // NOTE: this node no longer owns the odom->base_link TF. The scans it consumes are
  // muxed off during the 3D pitching sweep, so it deliberately goes quiet for tens of
  // seconds at a time. odom_gate_node re-publishes a continuous (and IMU-gated) TF from
  // the odometry topic, so nothing here needs to keep transforms alive artificially.
  if( rf2o_ref.is_initialized() && scan_available() )
  {
    // Process odometry estimation
    rf2o_ref.odometryCalculation(last_scan);

    // Publish odometry over ROS2 (topic; TF only if publish_tf was left enabled)
    publish();
  ...
  {
    // Expected while the scan mux is blinding us during a 3D sweep - keep it quiet.
